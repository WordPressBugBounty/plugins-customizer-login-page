...
/**
 * Backward compatible function to get page by title.
 * get_page_by_title() is deprecated since WordPress 6.2.
 *
 * @param string $page_title The page title to search for.
 * @return WP_Post|null The page object or null if not found.
 */
function lpc_get_page_by_title_compat($page_title)
{
	global $wpdb;

	// For WordPress 6.2+ use WP_Query.
	if (function_exists('wp_parse_args') && version_compare(get_bloginfo('version'), '6.2', '>=')) {
		$query = new WP_Query(
			array(
				'post_type'              => 'page',
				'title'                  => $page_title,
	...
	$page = $wpdb->get_var(
		$wpdb->prepare(
			"SELECT ID FROM $wpdb->posts WHERE post_title = %s AND post_type = 'page' LIMIT 1",
			$page_title
		)
	);
...
if (is_multisite()) {
	...
	{
		// Check page exists (backward compatible - get_page_by_title deprecated in WP 6.2).
		$lpc_exist_page = lpc_get_page_by_title_compat('Customizer Login Page Plugin');
...
?>
	...
	<style>
		...
		.loginpc-grid-item .image-link {
			...
			color: inherit;
			/* Inherit text color to prevent default anchor tag color */
		}
...
function loginpc_hide_other_panels($components)
{
	...
	// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- nonce not needed for display logic
	$loginpc_customize = isset($_GET['loginpc_customize']) ? sanitize_text_field(wp_unslash($_GET['loginpc_customize'])) : '';
	if ($loginpc_customize === 'true') {
...
function lpc_import_customizer_settings()
{
	...
	if (! isset($_FILES['lpc-import-file']) || ! is_uploaded_file($_FILES['lpc-import-file']['tmp_name'])) {
		wp_send_json_error(array('message' => 'No file uploaded.'));
		return;
	}

	// Sanitize file name.
	$file_name = isset($_FILES['lpc-import-file']['name']) ? sanitize_file_name($_FILES['lpc-import-file']['name']) : '';
	$tmp_name  = $_FILES['lpc-import-file']['tmp_name'];
	...
	$settings = $wp_filesystem->get_contents($tmp_name);

	// Validate content is not empty.
	if (empty($settings)) {
		wp_send_json_error(array('message' => 'Empty file.'));
		return;
	}
	...
	// Sanitize each setting value before saving.
	$sanitized_settings = lpc_sanitize_import_settings($decoded_settings);

	update_option('lpc_opts', $sanitized_settings);
	wp_send_json_success();
}

/**
 * Sanitize imported settings from JSON file.
 *
 * @param array $settings The settings array to sanitize.
 * @return array Sanitized settings array.
 */
function lpc_sanitize_import_settings($settings)
{
	...
	foreach ($settings as $key => $value) {
		...
		if (is_array($value)) {
			$sanitized[$clean_key] = lpc_sanitize_import_settings($value);
		} elseif (is_numeric($value)) {
			// Preserve numeric values.
			$sanitized[$clean_key] = is_float($value) ? floatval($value) : intval($value);
		} elseif (preg_match('/^(#[a-fA-F0-9]{3,8}|rgba?\s*\(|hsla?\s*\()/i', trim($value))) {
			// Sanitize color values (hex, rgba, etc.).
			$sanitized[$clean_key] = sanitize_text_field($value);
		} elseif (stripos($clean_key, 'image') !== false || stripos($clean_key, 'video') !== false || stripos($clean_key, 'url') !== false) {
			// Sanitize URLs.
			$sanitized[$clean_key] = esc_url_raw($value);
		} elseif (stripos($clean_key, 'css') !== false) {
			// Allow CSS values but strip dangerous content.
...
?>
	<style type="text/css">
		body.login {
			...
			<?php if ($lpc_bg_color_choice == 'solid') { ?>background-color: <?php echo esc_attr($lpc_bg_solid_color); ?>;
			<?php } elseif (($lpc_bg_color_choice == 'gradient') && empty($lpc_bg_image)) { ?>background-color: <?php echo esc_attr($lpc_bg_solid_color); ?>;
